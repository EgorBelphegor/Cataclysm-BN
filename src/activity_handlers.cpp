...
void activity_handlers::lockpicking_finish( player_activity *act, player *p )
{
    ...
    int pick_roll = 1.8 * ( std::pow( 1.6, p->get_skill_level( skill_mechanics ) ) +
                    it->get_quality( qual_LOCKPICK ) - it->damage() / 2000.0 ) + p->dex_cur / 4.0;
    int lock_roll = rng( 1, 120 );
    if( pick_roll >= lock_roll ) {
        p->practice(skill_mechanics, lock_roll );
